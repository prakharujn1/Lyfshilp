...
export const updateComputersPerformance = async (req, res) => {
  const {
    userId,
    score,
    accuracy,
  ...
  try {
    ...
    const existing = await prisma.computersPerformance.findUnique({
      where: { userId },
    });
    ...
    // Final upsert
    const result = await prisma.computersPerformance.upsert({
      where: { userId },
      update: updateData,
      create: {
        userId,
    ...
    res.status(500).json({ error: 'Computers performance update failed' });
  }
};
