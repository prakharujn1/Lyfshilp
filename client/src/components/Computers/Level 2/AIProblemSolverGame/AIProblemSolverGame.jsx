        ...
        <div className="p-6 max-w-6xl mx-auto text-center bg-gradient-to-br from-yellow-100 via-pink-100 to-blue-100 min-h-screen rounded-xl shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {problems.map((problem) => (
                    <motion.div
                        key={problem.id}
                        className={`bg-white p-6 rounded-3xl border-4 shadow-xl text-left transition duration-300 hover:shadow-2xl ${assignedTools[problem.id] ? "border-green-400 bg-green-50" : "border-yellow-300"
                            }`}
                    ...
                    >
                        ...
                        <p className="text-lg mb-4 italic text-gray-800">🧐 {problem.description}</p>
                        {assignedTools[problem.id] ? (
                            <>
                                <div className="text-xl mt-4 bg-green-100 p-2 rounded-lg">
                                    ✅ You picked: <strong>{assignedTools[problem.id].icon} {assignedTools[problem.id].label}</strong>
                                </div>
                                <textarea
                            ...
                            <p className="italic text-gray-500">🖱️ Click to let an AI buddy jump in!</p>
                        )}
                    </motion.div>
                ))}
                ...
                submitted && (
                    ...
                    >
                        ...
                        <ul className="text-left text-lg space-y-6">
                            {problems.map((problem) => (
                                <li key={problem.id} className="bg-white p-4 rounded-xl shadow-lg border border-gray-300">
                                    <strong className="text-2xl">{problem.title}</strong> — {problem.description} <br />
                                    🤖 <span className="font-bold">AI Tool Deployed:</span> {assignedTools[problem.id].icon} {assignedTools[problem.id].label}<br />
                                    💬 <span className="font-bold">Your Genius Reason:</span> {howItHelps[problem.id]} <br />
                                    🎯 <span className="font-bold">AI’s Secret Sauce:</span> {toolHelps[assignedTools[problem.id].id]}
                                </li>
                            ))}
                        </ul>
                    </motion.div>
