...
const WhatWentWrongGame = () => {
  ...
  return (
    <button
      ...
      className={`px-1 py-0.5 rounded-md transition-all duration-300 mx-0.5 ${
        isSelected
      ...
      <div className="flex flex-wrap justify-center mt-3 gap-4">
        {barriers.map((barrier) => (
          <motion.button
            ...
            className={`px-5 py-2 text-white rounded-full font-semibold shadow-md transition-all duration-300 ${
              selections[scenario.id]?.selectedBarrier === barrier
      ...
      {showResult && (
        <motion.p
          className={`mt-4 text-xl font-bold text-center ${
            isCorrect(scenario.id) ? 'text-green-600' : 'text-red-500'
          }`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
